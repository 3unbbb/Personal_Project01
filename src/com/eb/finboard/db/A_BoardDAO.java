...
public class A_BoardDAO {
	//DB에 eb_fin_board 테이블 관련된 모든 동작을 처리
	...
	//insertA_Board(bb) 시작
	public void insertBoard(A_BoardDTO dto){
		int num = 0; //글번호 저장 변수
		try {
			...
			sql = "select max(num) from eb_fin_board";
			...
			sql = "insert into eb_fin_board(num,id,department,subject,content,"
	...
	public int getBoardCount() {
		...
		try {
			...
			//sql
			sql = "select count(num) from eb_fin_board";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList getBoardList(){
		...
		try {
			...
			//sql & pstmt
			sql = "select * from eb_fin_board order by num desc limit 0,5";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList getBoardList(int startRow, int pageSize){
		...
		try {
			...
			//sql & pstmt
			sql = "select * from eb_fin_board order by re_ref desc, re_seq asc limit ?,?";
	...
	public void updateReadCount(int num){
		...
		try {
			...
			sql = "update eb_fin_board set read_count = read_count+1  where num = ?";
			pstmt = con.prepareStatement(sql);
	...
	//getA_Board(num)
	public A_BoardDTO getBoard(int num){
		A_BoardDTO dto = null;
		...
		try {
			...
			con = getCon();
			// 3. sql 작성 & pstmt 객체
			sql = "select * from eb_fin_board where num = ?";
			pstmt = con.prepareStatement(sql);
	...
	//modifyA_Board(num)
	public A_BoardDTO modifyBoard(int num){
		A_BoardDTO dto = null;
		try {
			//디비연결
			con=getCon();
	...
	//updateA_Board(num, dto)
	public void updateBoard(int num, A_BoardDTO dto){
		try {
		
		//1. 디비연결
		con = getCon();
		
		sql = "update eb_fin_board "
				+ "set subject =?, content=? "
				+ "where num= ?";
	...
	public void deleteBoard(int num){
		...
		try {
			...
			sql="delete from eb_fin_board where num=?";
			pstmt = con.prepareStatement(sql);
	...
	//boardReinsert(num)
		public void boardReinsert(A_BoardDTO dto){
			...
			try {
				...
				sql = "update eb_fin_board set re_seq = re_seq + 1 "
