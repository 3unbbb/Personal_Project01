...
public class H_BoardDAO {
	//DB에 eb_hr_board 테이블 관련된 모든 동작을 처리
	...
	public void insertH_Board(H_BoardDTO dto){
		...
		try {
			...
			sql = "select max(num) from eb_hr_board";
			...
			sql = "insert into eb_hr_board(num,id,department,subject,content,"
	...
	public int getH_BoardCount() {
		...
		try {
			...
			//sql
			sql = "select count(num) from eb_hr_board";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList getH_BoardList(){
		...
		try {
			...
			//sql & pstmt
			sql = "select * from eb_hr_board order by num desc limit 0,5";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList getH_BoardList(int startRow, int pageSize){
		...
		try {
			...
			//sql & pstmt
			sql = "select * from eb_hr_board order by re_ref desc, re_seq asc limit ?,?";
	...
	public void updateReadCount(int num){
		...
		try {
			...
			sql = "update eb_hr_board set read_count = read_count+1  where num = ?";
			pstmt = con.prepareStatement(sql);
	...
	public H_BoardDTO getH_Board(int num){
		...
		try {
			...
			con = getCon();
			// 3. sql 작성 & pstmt 객체
			sql = "select * from eb_hr_board where num = ?";
			pstmt = con.prepareStatement(sql);
	...
	public void deleteH_Board(int num){
		...
		try {
			...
			sql="delete from eb_hr_board where num=?";
			pstmt = con.prepareStatement(sql);
	...
	//boardReinsert(num)
		public void boardReinsert(H_BoardDTO dto){
			...
			try {
				...
				sql = "update eb_hr_board set re_seq = re_seq + 1 "
